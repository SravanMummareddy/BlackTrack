...
// Loaded lazily from GET /strategy/chart on first render.
let chartData = null;
...
function renderChartPreview() {
  if (!chartData) {
    loadStrategyChart();
    return `<div class="empty-state"><h3>Loading reference chart</h3><p>Pulling the canonical basic-strategy chart from the API.</p></div>`;
  }
  return `
...
async function loadStrategyChart() {
  ...
  chartLoading = true;
  try {
    chartData = await api("/strategy/chart", {}, false);
    render();
  } catch (error) {
    addNotice(error.message || "Could not load the strategy chart.", "error");
  } finally {
    chartLoading = false;
  }
}
